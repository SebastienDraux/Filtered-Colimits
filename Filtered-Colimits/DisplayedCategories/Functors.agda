...
module _ {C : Category ℓC ℓC'}
  ...
  preservesCocartMor : dispCat-Funct D D' → Type (ℓ-max (ℓ-max ℓC ℓD) (ℓ-max ℓC' ℓD'))
  preservesCocartMor 𝑭 = {x y : ob C} → {f : C [ x , y ]} → {X : D ⦅ x ⦆} → {Y : D ⦅ y ⦆} → (F : D [ f , X , Y ]ᴰ) → isCocartesian D f X Y F → isCocartesian D' f (𝑭 ⟅ X ⟆ᴰ) (𝑭 ⟅ Y ⟆ᴰ) (𝑭 ⟪ F ⟫ᴰ)
  isProp-preservesCocartMor : (𝑭 : dispCat-Funct D D') → isProp (preservesCocartMor 𝑭)
  isProp-preservesCocartMor 𝑭 = isPropImplicitΠ2 (λ _ _ → isPropImplicitΠ (λ _ →  isPropImplicitΠ2 (λ _ _ → isPropΠ2 (λ F _ → isProp-isCocartesian D' (𝑭 ⟪ F ⟫ᴰ)))))
...
module _ {C : Category ℓC ℓC'}
  ...
  module _ {𝑭 𝑮 : dispCat-Funct D D'} where
    ...
    ≡eq-dF eqFG eqFG' ≡eq-dF-ob i .eq-dF-hom {f = f} {X} {Y} F = isProp→PathP
    ...
    ≡→eq-dF p .eq-dF-hom {X = X} {Y} F = sym (dC-substHomLR D' (cong (_⟅ X ⟆ᴰ) p) (cong (_⟅ Y ⟆ᴰ) p) (𝑭 ⟪ F ⟫ᴰ)) ∙ fromPathP (λ i → (p i) ⟪ F ⟫ᴰ)
   
    ≡→eq-dF→≡ : (p : 𝑭 ≡ 𝑮) → eq-dF→≡ (≡→eq-dF p) ≡ p
    ≡→eq-dF→≡ p i j .dF-ob X = (p j) ⟅ X ⟆ᴰ
    ≡→eq-dF→≡  p i j .dF-hom {f = f} {X} {Y} F =
