...
module _ {C : Category ℓC ℓC'}
    ...
    q = 
      ...
      subst (λ f → D [ f , _ , _ ]ᴰ) (⋆IdR C _) (subst (λ f → D [ f , _ , _ ]ᴰ) (cong (λ g → inv f ⋆⟨ C ⟩ g) (ret f)) (dC-inv F ⋆⟨ D ⟩ᴰ (dC-mor G ⋆⟨ D ⟩ᴰ dC-inv G)))
       ...
       subst (λ f → D [ f , _ , _ ]ᴰ) (cong (λ g → inv f ⋆⟨ C ⟩ g) (ret f) ∙ ⋆IdR C _) (dC-inv F ⋆⟨ D ⟩ᴰ (dC-mor G ⋆⟨ D ⟩ᴰ dC-inv G))
         ≡⟨ cong (λ F → subst (λ f → D [ f , _ , _ ]ᴰ) (cong (λ g → inv f ⋆⟨ C ⟩ g) (ret f) ∙ ⋆IdR C _) F) (sym (dC-⋆Assoc D _ _ _)) ⟩
  ...
  isSetDispCatIso : {x y : ob C} → (f : CatIso C x y) → (X : D ⦅ x ⦆) → (Y : D ⦅ y ⦆) → isSet (dispCatIso f X Y)
  isSetDispCatIso f X Y F G p q i j .dC-mor = isSet→SquareP {A = λ _ _ → D [ mor f , _ , _ ]ᴰ} (λ _ _ → dC-homSet D _ _ _) (λ j → dC-mor (p j)) (λ j → dC-mor (q j)) refl refl i j
  isSetDispCatIso f X Y F G p q i j .dC-inv = isSet→SquareP {A = λ _ _ → D [ inv f , _ , _ ]ᴰ} (λ _ _ → dC-homSet D _ _ _) (λ j → dC-inv (p j)) (λ j → dC-inv (q j)) refl refl i j
  isSetDispCatIso f X Y F G p q i j .dC-sec =
    isSet→SquareP {A = λ i j → subst (λ f → D [ f , _ , _ ]ᴰ) (sec f) (isSetDispCatIso f X Y F G p q i j .dC-inv ⋆⟨ D ⟩ᴰ isSetDispCatIso f X Y F G p q i j .dC-mor) ≡ dC-id D}
    (λ i j → isProp→isSet (dC-homSet D _ _ _ _ _)) (λ j → dC-sec (p j)) (λ j → dC-sec (q j)) refl refl i j
  isSetDispCatIso f X Y F G p q i j .dC-ret = 
    isSet→SquareP {A = λ i j → subst (λ f → D [ f , _ , _ ]ᴰ) (ret f) (isSetDispCatIso f X Y F G p q i j .dC-mor ⋆⟨ D ⟩ᴰ isSetDispCatIso f X Y F G  p q i j .dC-inv) ≡ dC-id D}
    (λ i j → isProp→isSet (dC-homSet D _ _ _ _ _)) (λ j → dC-ret (p j)) (λ j → dC-ret (q j)) refl refl i j


  idDispCatIso : {x : ob C} → {X : D ⦅ x ⦆} → dispCatIso idCatIso X X
  ...
  dC-pathToIso : {x : ob C} → {X X' : D ⦅ x ⦆} → X ≡ X' → dispCatIso idCatIso X X'
  dC-pathToIso {X = X} p = J (λ X' p → dispCatIso idCatIso X X') idDispCatIso p

  dC-pathToIsoRefl : {x : ob C} → {X : D ⦅ x ⦆} → dC-pathToIso refl ≡ idDispCatIso {X = X}
  dC-pathToIsoRefl {X = X} = JRefl (λ X' p → dispCatIso idCatIso X X') idDispCatIso
  

  isUnivalent-dC : Type (ℓ-max (ℓ-max ℓC ℓD) ℓD')
  isUnivalent-dC = {x : ob C} → (X X' : D ⦅ x ⦆) → isEquiv (dC-pathToIso {x = x} {X = X} {X' = X'})
    
  dC-univEquiv : {x : ob C} → isUnivalent-dC → (X X' : D ⦅ x ⦆) → (X ≡ X') ≃ (dispCatIso idCatIso X X')
