...
record Pre-2-Category ℓ ℓ' ℓ'' : Type (ℓ-suc (ℓ-max (ℓ-max ℓ ℓ') ℓ'')) where
  field
    ob₀ : Type ℓ
    ob₁[_,_] : ob₀ → ob₀ → Precategory ℓ' ℓ''
    id₁ : (x : ob₀) → ob ob₁[ x , x ]
    _⋆₁_ : {x y z : ob₀} → ob ob₁[ x , y ] → ob ob₁[ y , z ] → ob ob₁[ x , z ]
    ⋆₁IdL : {x y : ob₀} → (f : ob ob₁[ x , y ]) → (id₁ x) ⋆₁ f ≡ f
    ⋆₁IdR : {x y : ob₀} → (f : ob ob₁[ x , y ]) → f ⋆₁ (id₁ y) ≡ f
    ...
    ⋆₂IdL : {x y : ob₀} → {f g : ob ob₁[ x , y ]} → (α : ob₁[ x , y ] [ f , g ]) →
      id ob₁[ x , x ] (id₁ x) ⋆₂ α ≡ morP ob₁[ x , y ] (⋆₁IdL f) ⋆⟨ ob₁[ x , y ] ⟩ α ⋆⟨ ob₁[ x , y ] ⟩ invP ob₁[ x , y ] (⋆₁IdL g)
    ⋆₂IdR : {x y : ob₀} → {f g : ob ob₁[ x , y ]} → (α : ob₁[ x , y ] [ f , g ]) →
      α ⋆₂ id ob₁[ y , y ] (id₁ y) ≡ morP ob₁[ x , y ] (⋆₁IdR f) ⋆⟨ ob₁[ x , y ] ⟩ α ⋆⟨ ob₁[ x , y ] ⟩ invP ob₁[ x , y ] (⋆₁IdR g)
    ...
    ⋆₂Id : {x y z : ob₀} → (f : ob ob₁[ x , y ]) → (g : ob ob₁[ y , z ]) → id ob₁[ x , y ] f ⋆₂ id ob₁[ y , z ] g ≡ id ob₁[ x , z ] (f ⋆₁ g)
...
_[_,_]₁ : (C : Pre-2-Category ℓ ℓ' ℓ'') → (x y : C .ob₀) → Precategory ℓ' ℓ''
_[_,_]₁ = ob₁[_,_]

seq₁ : (C : Pre-2-Category ℓ ℓ' ℓ'') → {x y z : ob₀ C} → ob (C [ x , y ]₁) → ob (C [ y , z ]₁) → ob (C [ x , z ]₁)
seq₁ C = C ._⋆₁_ 

infixl 15 seq₁
syntax seq₁ C f g = f ⋆⟨ C ⟩₁ g

seq₂ : (C : Pre-2-Category ℓ ℓ' ℓ'') → (x y z : ob₀ C) → {f f' : ob (C [ x , y ]₁)} → {g g' : ob (C [ y , z ]₁)} → (α : C [ x , y ]₁ [ f , f' ]) → (β : C [ y , z ]₁ [ g , g' ]) → C [ x , z ]₁ [ f ⋆⟨ C ⟩₁ g , f' ⋆⟨ C ⟩₁ g' ]
seq₂ C x y z = C ._⋆₂_
...
record 2-Category ℓ ℓ' ℓ'' : Type (ℓ-suc (ℓ-max (ℓ-max ℓ ℓ') ℓ'')) where
  field
    2-cat : Pre-2-Category ℓ ℓ' ℓ''
    ⦃ isCat ⦄ : {x y : ob₀ 2-cat} → isCategory (2-cat [ x , y ]₁)
    
open Precategory

PrecatCat : {ℓ ℓ' : Level} → Pre-2-Category (ℓ-suc (ℓ-max ℓ ℓ')) (ℓ-max ℓ ℓ') (ℓ-max ℓ ℓ')
PrecatCat {ℓ} {ℓ'} .ob₀ = Category ℓ ℓ'
PrecatCat .ob₁[_,_] C D = FUNCTOR (cat C) (cat D)
PrecatCat .id₁ C = 𝟙⟨ cat C ⟩
PrecatCat ._⋆₁_ F G = F ⋆ᶠ G
PrecatCat .⋆₁IdL F = ⋆ᶠIdL F
PrecatCat .⋆₁IdR F = ⋆ᶠIdR F
PrecatCat .⋆₁Assoc F G H = ⋆ᶠAssoc F G H
PrecatCat ._⋆₂_ {C} {D} {E} {F} {F'} {G} {G'} α β .N-ob x = β ⟦ F ⟅ x ⟆ ⟧ ⋆⟨ cat E ⟩ G' ⟪ α ⟦ x ⟧ ⟫
PrecatCat ._⋆₂_ {C} {D} {E} {F} {F'} {G} {G'} α β .N-hom {x} {y} f =
 G ⟪ F ⟪ f ⟫ ⟫ ⋆⟨ cat E ⟩ (β ⟦ F ⟅ y ⟆ ⟧ ⋆⟨ cat E ⟩ G' ⟪ α ⟦ y ⟧ ⟫)
      ≡⟨ sym (⋆Assoc (cat E) (G ⟪ F ⟪ f ⟫ ⟫) (β ⟦ F ⟅ y ⟆ ⟧) (G' ⟪ α ⟦ y ⟧ ⟫)) ⟩
 (G ⟪ F ⟪ f ⟫ ⟫ ⋆⟨ cat E ⟩ β ⟦ F ⟅ y ⟆ ⟧) ⋆⟨ cat E ⟩ G' ⟪ α ⟦ y ⟧ ⟫
      ≡⟨ cong (λ g → g ⋆⟨ cat E ⟩ G' ⟪ α ⟦ y ⟧ ⟫) (N-hom β (F ⟪ f ⟫)) ⟩
 (β ⟦ F ⟅ x ⟆ ⟧ ⋆⟨ cat E ⟩ G' ⟪ F ⟪ f ⟫ ⟫) ⋆⟨ cat E ⟩ G' ⟪ α ⟦ y ⟧ ⟫
      ≡⟨ ⋆Assoc (cat E) (β ⟦ F ⟅ x ⟆ ⟧) (G' ⟪ F ⟪ f ⟫ ⟫) (G' ⟪ α ⟦ y ⟧ ⟫) ⟩
 β ⟦ F ⟅ x ⟆ ⟧ ⋆⟨ cat E ⟩ (G' ⟪ F ⟪ f ⟫ ⟫ ⋆⟨ cat E ⟩ G' ⟪ α ⟦ y ⟧ ⟫)
      ≡⟨ cong (λ g → β ⟦ F ⟅ x ⟆ ⟧ ⋆⟨ cat E ⟩ g) (sym (F-seq G' (F ⟪ f ⟫) (α ⟦ y ⟧))) ⟩
 β ⟦ F ⟅ x ⟆ ⟧ ⋆⟨ cat E ⟩ (G' ⟪ F ⟪ f ⟫ ⋆⟨ cat D ⟩ α ⟦ y ⟧ ⟫)
      ≡⟨ cong (λ g → β ⟦ F ⟅ x ⟆ ⟧ ⋆⟨ cat E ⟩ (G' ⟪ g ⟫)) (N-hom α f) ⟩
 β ⟦ F ⟅ x ⟆ ⟧ ⋆⟨ cat E ⟩ (G' ⟪ α ⟦ x ⟧ ⋆⟨ cat D ⟩ F' ⟪ f ⟫ ⟫)
      ≡⟨ cong (λ g → β ⟦ F ⟅ x ⟆ ⟧ ⋆⟨ cat E ⟩ g) (F-seq G' (α ⟦ x ⟧) (F' ⟪ f ⟫)) ⟩
 β ⟦ F ⟅ x ⟆ ⟧ ⋆⟨ cat E ⟩ (G' ⟪ α ⟦ x ⟧ ⟫ ⋆⟨ cat E ⟩ G' ⟪ F' ⟪ f ⟫ ⟫)
      ≡⟨ sym (⋆Assoc (cat E) (β ⟦ F ⟅ x ⟆ ⟧) (G' ⟪ α ⟦ x ⟧ ⟫) (G' ⟪ F' ⟪ f ⟫ ⟫)) ⟩
 (β ⟦ F ⟅ x ⟆ ⟧ ⋆⟨ cat E ⟩ G' ⟪ α ⟦ x ⟧ ⟫) ⋆⟨ cat E ⟩ G' ⟪ F' ⟪ f ⟫ ⟫ ∎
PrecatCat .⋆₂IdL {C} {D} {F} {G} α = makeNatTransPath (funExt eq)
  where
  eq : (x : ob (cat C)) →
    α ⟦ x ⟧ ⋆⟨ cat D ⟩ G ⟪ id (cat C) x ⟫ ≡ (morP (FUNCTOR (cat C) (cat D)) (makeFunctPathRefl (F ∘F 𝟙⟨ cat C ⟩) (F-id F) (F-seq F)) ⟦ x ⟧ ⋆⟨ cat D ⟩ α ⟦ x ⟧) ⋆⟨ cat D ⟩ (invP (FUNCTOR (cat C) (cat D)) (makeFunctPathRefl (funcComp G 𝟙⟨ cat C ⟩) (F-id G) (F-seq G))) ⟦ x ⟧
  eq x = 
